...
function mostrarArreglo(v: number[], n: number) {
  let i: number;
  for (i = 0; i < n; i++) {
    console.log("se cargo el numero: " + v[i], "en la posicion: " + (i + 1));
  }
}
function ejec() {
  ...
  cargarValores(v2, n);
  mostrarArreglo(v2, n);
  //alert("se esta por ejecutar la suma de los elementos de cada una de las posiciones de V1 y V2 ¿desea proceder?");
...
//alert("usted esta por ejecutar una calculadora de los elementos de cada posicion de un ARRAY ¿desea proceder?");
//alert("usted esta por cargar el valor de N (longitud del array) ¿desea proceder?");
cargarN(n);
